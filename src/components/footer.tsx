...
export function Footer() {
  return (
    <footer className="border-t border-white/10 bg-black">
      <div className="px-[30px] pt-10 pb-8 flex flex-col gap-8">

        {/* Top row — brand + social icons */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="flex flex-col gap-1">
            <span
              className="font-bold text-white text-[13px] tracking-tight"
          ...
          <ul className="flex flex-wrap gap-x-6 gap-y-2">
            {footerLinks.map((link) => (
              <li key={link.label}>
                <a
                  href={link.href}
                  className="text-[10px] text-white/25 hover:text-white uppercase tracking-widest transition-colors"
                  style={{ fontFamily: "var(--mono)" }}
                >
                  {link.label}
                </a>
